...
def extract_all_products(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None) -> List[Dict[str, Any]]:
    """
    One product row per <substanceAdministration classCode="SBADM" moodCode="EVN"> (document order).
    Read details starting at that SA node and DO NOT aggregate across multiple SAs in the same component.
    Other extraction logic is preserved but scoped to the SA subtree.
    """
    # Existing classification sets (unchanged logic)
    ...
    # Iterate components, but emit one row per SA in document order
    ...
    for cidx, comp in enumerate(comps, start=1):
        ...
        for sa in sa_nodes:
            ...
            # ---- Identify this product (ID + Name) ----
            id_el = find_first(sa, './/hl7:id')
            pid = (id_el.attrib.get('root') or '').strip() if id_el is not None else ''
            title = clean_value(_resolve_drug_name(sa)) or (pid or f"sa::{cidx:03d}-{sa_counter:03d}")

            # ---- Tags (Suspect / Interacting / Treatment / Concomitant) ----
            tags = set()
            if pid and pid in suspects:
                tags.add("Suspect")
            if pid and pid in interact:
                tags.add("Interacting")
            if pid and pid in treatments:
                tags.add("Treatment")
            if not tags:
            ...
            # ---- Start / Stop dates (low/high only; keep logic same) ----
            ...
            # ---- MAH (search common places under this SA only) ----
            mah = ""
            for xp in [
            ...
            # ---- Action Taken (scan within this SA scope) ----
            action_taken_vals: List[str] = []
            for act_code in sa.findall('.//hl7:act[@classCode="ACT"][@moodCode="EVN"]/hl7:code', NS):
            ...
            # ---- Drug Obtain Country (within this SA) ----
            obtain_countries: List[str] = []
            for cn in sa.findall('.//hl7:country', NS):
                val = (cn.text or '').strip()
                if val and val not in obtain_countries:
                    obtain_countries.append(val)
            obtain_country = "\n".join(obtain_countries)

            # ---- Indication (STATUS_OID + displayName='indication' or code='19'), scoped to this SA ----
            indications: List[str] = []
            for ind_obs in sa.findall('.//hl7:observation', NS):
                ...
                if cs == STATUS_OID and (dn == 'indication' or cd == '19'):
                    ...
                    if val is not None:
                        ...
                        if ot is not None and (ot.text or '').strip():
                            rrt = ot.text.strip()
                        # Display rule (term if mapped, else code)
                        llt_display = ""
                        if meddra_map and llt_code in meddra_map:
            ...
            out.append({
                ...
                "_gid": f"pid::{pid.lower()}" if pid else f"sa::{cidx:03d}-{sa_counter:03d}",
                "_pid": pid or "",
            })
