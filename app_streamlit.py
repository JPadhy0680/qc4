        ...
        for c in row.findall('a:c', ns):
            ...
            if t == 's':
                if v is not None and v.text and v.text.isdigit():
                    ss_idx = int(v.text)
                    if 0 <= ss_idx < len(shared_strings): value = shared_strings[ss_idx]
...
def _read_xlsx_no_openpyxl(uploaded_file) -> pd.DataFrame:
    ...
    try:
        ...
        for si in sroot.findall('.//a:si', s_ns):
            parts = [tnode.text or '' for tnode in si.findall('.//a:t', s_ns)]
    ...
    if sheet_bytes is None: raise ValueError("No worksheet XML found in XLSX.")
    return _parse_sheet_xml(sheet_bytes, shared_strings)

# ---- MedDRA mapping loader (no openpyxl needed) ----
...
def get_pq_value_by_code(root: ET.Element, display_name: Optional[str] = None, code_system_oid: Optional[str] = None) -> Tuple[str, str]:
    for obs in root.findall('.//hl7:observation', NS):
        ...
        ok = False
        if display_name and (code_el.attrib.get('displayName') or '').strip().lower() == display_name.lower(): ok = True
        if (not ok) and code_system_oid and (code_el.attrib.get('codeSystem') == code_system_oid): ok = True
        if not ok: continue
...
def extract_patient(root: ET.Element) -> Dict[str, str]:
    ...
    if clean_value(age_val):
        ...
        if clean_value(age_unit_label): age = f"{age} {age_unit_label}"
    ...
    if not (w_val or w_unit):
        for obs in root.findall('.//hl7:observation', NS):
            ...
            if u in {'kg','lb','lbs'}:
                ...
                if w_val: break
    weight = f"{clean_value(w_val)} {clean_value(w_unit)}".strip()
    ...
    if not (h_val or h_unit):
        for obs in root.findall('.//hl7:observation', NS):
            ...
            if u in {'cm','m','in'}:
                ...
                if h_val: break
    height = f"{clean_value(h_val)} {clean_value(h_unit)}".strip()
    initials = ""
...
def build_reaction_id_to_term(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None) -> Dict[str, str]:
    ...
    for obs in findall(root, './/hl7:observation[@classCode="OBS"][@moodCode="EVN"]'):
        code_el = obs.find('hl7:code', NS)
        if code_el is None or (code_el.attrib.get('displayName') or '').strip().lower() != 'reaction': continue
        ...
        if not llt_term and val_el is not None:
            ...
            if not llt_term:
                ...
                if ot is not None and (ot.text or '').strip(): llt_term = ot.text.strip()
        if llt_term:
...
def extract_medical_history(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None, debug: bool = False) -> List[Dict[str, Any]]:
    ...
    for code in root.findall('.//hl7:code', NS):
        ...
        if c == "1" or disp == "relevantmedicalhistoryandconcurrentconditions": anchors.append(code)
    for anc in anchors:
        ...
        for obs in container.findall('.//hl7:observation[@classCode="OBS"][@moodCode="EVN"]', NS):
            ...
            if meddra_map and llt_code in meddra_map:
                m = meddra_map[llt_code]; llt_term = (m.get("LLT Term") or "").strip(); pt_code = (m.get("PT Code") or "").strip(); pt_term = (m.get("PT Term") or "").strip()
            if not llt_term:
            ...
            for inb in obs.findall('.//hl7:inboundRelationship/hl7:observation', NS):
                scode = inb.find('hl7:code', NS); val = inb.find('hl7:value', NS)
...
def extract_labs(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None, debug: bool = False) -> List[Dict[str, Any]]:
    ...
    for code in root.findall('.//hl7:code', NS):
        ...
        c = (code.attrib.get('code') or '').strip()
        disp = (code.attrib.get('displayName') or '').strip().lower()
        if c == "3" or disp == "testsandproceduresrelevanttotheinvestigation": anchors.append(code)
    for anc in anchors:
        container = pmap.get(anc, None) or root
        for obs in container.findall('.//hl7:observation[@classCode="OBS"][@moodCode="EVN"]', NS):
            code = obs.find('hl7:code', NS)
            if code is None or (code.attrib.get('codeSystem') or '').strip() != MEDDRA_LLT_OID: continue
            llt_code = (code.attrib.get('code') or '').strip()
            llt_term, pt_code, pt_term = "", "", ""
            if meddra_map and llt_code in meddra_map:
                m = meddra_map[llt_code]; llt_term = (m.get("LLT Term") or "").strip(); pt_code = (m.get("PT Code") or "").strip(); pt_term = (m.get("PT Term") or "").strip()
            ...
            more_info = ""
            for outb in obs.findall('.//hl7:outboundRelationship/hl7:observation', NS):
                scode = outb.find('hl7:code', NS); bval  = outb.find('hl7:value', NS)
                if scode is not None and (scode.attrib.get('codeSystem') or '').strip() == STATUS_OID:
                    label = (scode.attrib.get('displayName') or scode.attrib.get('code') or '').strip()
                    if label.lower() == "moreinformationavailable" and bval is not None:
                        bl = (bval.attrib.get('value') or '').strip().lower()
                        if bl in {"true","false"}: more_info = "Yes" if bl == "true" else "No"; break
            key = llt_code or normalize_text(llt_term)
            if not key: continue
            items.append({
                "LLT Code": clean_value(llt_code), "LLT Term": clean_value(llt_term),
                "PT Code": clean_value(pt_code),   "PT Term": clean_value(pt_term),
                "Result": clean_value(result),     "Result Date": clean_value(date_val),
...
def extract_causality(root: ET.Element, product_id_to_name: Optional[Dict[str, str]] = None,
                      reaction_id_to_term: Optional[Dict[str, str]] = None, debug: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for node in findall(root, './/hl7:causalityAssessment[@classCode="OBS"][@moodCode="EVN"]'):
            ...
            if not (dsn == 'causality' or cd == '39'): continue
            val = find_first(node, './/hl7:value')
            assessment = (val.attrib.get('value') or "").strip() if val is not None else ""
            if not assessment and val is not None: assessment = get_text(val)
            method = get_text(find_first(node, './/hl7:methodCode/hl7:originalText'))
            assessor = ""
            nm = find_first(node, './/hl7:author//hl7:assignedEntity//hl7:name')
            if nm is not None: assessor = get_text(nm)
            if not assessor: assessor = get_text(find_first(node, './/hl7:author//hl7:assignedEntity//hl7:originalText'))
            evt_id = ""; prd_id = ""
            evt = find_first(node, './/hl7:subject1//hl7:adverseEffectReference//hl7:id')
            if evt is not None: evt_id = (evt.attrib.get('root') or '').strip() or (evt.attrib.get('extension') or '').strip()
            prd = find_first(node, './/hl7:subject2//hl7:productUseReference//hl7:id')
            if prd is not None: prd_id = (prd.attrib.get('root') or '').strip() or (prd.attrib.get('extension') or '').strip()
            ...
            out.append({
                "Assessment": clean_value(assessment), "Method": clean_value(method), "Assessor": clean_value(assessor),
                "Reaction": clean_value(reaction_name), "Drug": clean_value(drug_name),
                "_key": key, "_evt_id": evt_id, "_prd_id": prd_id,
            })
    except Exception as e:
        if debug: st.warning(f"Causality parse error: {e}")
    return out

# ---------------- Products extraction (grouped by COMPONENT; NO REGIMENS) ----------------
...
def extract_interacting_ids(root: ET.Element) -> Set[str]:
    ...
    for obs in findall(root, './/hl7:observation'):
        ...
        if 'interact' in disp:
            ...
            if ref is not None: ids.add(ref.attrib.get('root',''))
    return ids
...
def _add_unique(acc_list: List[str], value: str):
    v = clean_value(value)
    if not v: return
    if v not in acc_list: acc_list.append(v)
...
def extract_all_products(root: ET.Element) -> List[Dict[str, Any]]:
    ...
    for cidx, comp in enumerate(comps, start=1):
        ...
        for sa in sas:
            ...
            if id_el is not None and (id_el.attrib.get('root') or '').strip():
                pid = id_el.attrib.get('root').strip(); break
        title = ""
        for sa in sas:
            nm = _resolve_drug_name(sa).strip()
            if nm: title = clean_value(nm); break
        if not title: title = pid or f"comp::{cidx:03d}"
        tags = set()
        if pid and pid in suspects:   tags.add("Suspect")
        if pid and pid in interact:   tags.add("Interacting")
        if pid and pid in treatments: tags.add("Treatment")
        if not tags:                  tags.add("Concomitant")
        type_disp = ", ".join(sorted(tags))
        agg = {"Dosage Text": [],"Dose Value": [],"Dose Unit": [],"Start Date": [],"Stop Date": [],
               "Route": [],"Formulation": [],"Lot No": [],"MAH": [],"Action Taken": [],"Drug Obtain Country": []}
        for sa in sas:
            txt = get_text(find_first(sa, './/hl7:text')); _add_unique(agg["Dosage Text"], txt)
            ...
            low  = find_first(sa, './/hl7:effectiveTime/hl7:low', NS)
            high = find_first(sa, './/hl7:effectiveTime/hl7:high', NS)
            ...
            _add_unique(agg["Start Date"], sd); _add_unique(agg["Stop Date"], ed)
            rtxt = get_text(find_first(sa, './/hl7:routeCode/hl7:originalText'))
            if not rtxt:
                rc = find_first(sa, './/hl7:routeCode'); rtxt = (rc.attrib.get('displayName') or '') if rc is not None else ''
            _add_unique(agg["Route"], rtxt)
            form = get_text(find_first(sa, './/hl7:formCode/hl7:originalText')); _add_unique(agg["Formulation"], form)
            lot  = get_text(find_first(sa, './/hl7:lotNumberText'));            _add_unique(agg["Lot No"], lot)
            mah  = ""
            for p in ['.//hl7:playingOrganization/hl7:name','.//hl7:manufacturerOrganization/hl7:name',
                      './/hl7:asManufacturedProduct/hl7:manufacturerOrganization/hl7:name']:
                node = find_first(sa, p)
                if node is not None and get_text(node): mah = get_text(node); break
        ...
        for act_code in comp.findall('.//hl7:act[@classCode="ACT"][@moodCode="EVN"]/hl7:code', NS):
            if (act_code.attrib.get('codeSystem') or '').strip() == ACTION_TAKEN_OID:
                ...
                if label: _add_unique(agg["Action Taken"], label)
        for cn in comp.findall('.//hl7:country', NS):
            val = (cn.text or '').strip()
            if val: _add_unique(agg["Drug Obtain Country"], val)
        def join_vals(lst: List[str]) -> str: return "\n".join([v for v in lst if has_value(v)])
        out.append({
            "Drug": title, "Type": type_disp,
            "Dosage Text": join_vals(agg["Dosage Text"]), "Dose Value": join_vals(agg["Dose Value"]),
            "Dose Unit": join_vals(agg["Dose Unit"]), "Start Date": join_vals(agg["Start Date"]),
            "Stop Date": join_vals(agg["Stop Date"]), "Route": join_vals(agg["Route"]),
            "Formulation": join_vals(agg["Formulation"]), "Lot No": join_vals(agg["Lot No"]),
            "MAH": join_vals(agg["MAH"]), "Action Taken": join_vals(agg["Action Taken"]),
            "Drug Obtain Country": join_vals(agg["Drug Obtain Country"]),
            "_gid": f"pid::{pid.lower()}" if pid else f"comp::{cidx:03d}", "_pid": pid or "",
        })
    return out

# ---------------- Reporter extraction (STRICT: only sourceReport branches) ----------------
def find_all_source_report_containers(root: ET.Element) -> List[ET.Element]:
    ...
    def ancestors(node: ET.Element) -> List[ET.Element]:
        acc = []; cur = node
        while cur in parent: cur = parent[cur]; acc.append(cur)
        return acc
    ...
    for code_el in code_nodes:
        for anc in ancestors(code_el):
            ...
            if lname in {'relatedInvestigation', 'subjectOf2', 'controlActEvent'}:
                for xp in ['.//hl7:author/hl7:assignedEntity','.//hl7:author/hl7:assignedAuthor','.//hl7:informant/hl7:assignedEntity']:
                    cand = anc.find(xp, NS)
                    if cand is not None: containers.append(cand)
                break
    seen, uniq = set(), []
    for el in containers:
        if id(el) not in seen: seen.add(id(el)); uniq.append(el)
    return uniq

def extract_reporter_from_container(node: ET.Element) -> Dict[str, str]:
    result = {
        "Reporter Qualification": "","Reporter IDs": "","Reporter Title": "","Reporter Given Name(s)": "",
        "Reporter Family Name": "","Reporter Organization": "","Reporter Street": "","Reporter City/Town": "",
        "Reporter State/Province": "","Reporter Postal Code": "","Reporter Country": "",
        "Reporter Phone(s)": "","Reporter Email(s)": "","Reporter Fax(es)": "",
    }
    ids = []
    for id_el in node.findall('.//hl7:id', NS):
        ext = (id_el.attrib.get('extension') or '').strip(); rt  = (id_el.attrib.get('root') or '').strip()
    ...
    for code_el in node.iter():
        if local_name(code_el.tag) == 'code' and code_el.attrib.get('codeSystem') == REPORTER_QUAL_OID:
            c = (code_el.attrib.get('code') or '').strip(); qual = REPORTER_MAP.get(c, c); break
    ...
    for xp in ['.//hl7:assignedEntity/hl7:representedOrganization/hl7:name',
               './/hl7:representedOrganization/hl7:name','.//hl7:scopingOrganization/hl7:name']:
        el = node.find(xp, NS)
        if el is not None:
    ...
    if not country:
        ...
        if loc is not None and loc.attrib.get('code'): country = loc.attrib.get('code').strip()
    ...
    for tel in node.findall('.//hl7:telecom', NS):
        raw = (tel.attrib.get('value') or '').strip(); use = (tel.attrib.get('use') or '').upper()
        if not raw: continue
        low = raw.lower()
        if low.startswith('mailto:'):
            ...
            if '@' in raw: emails.append(raw.replace('mailto:', ''))
                ...
                phones.append(raw if len(digits) >= 7 else raw)
...
def extract_reporters_from_sourceReport(root: ET.Element) -> List[Dict[str, str]]:
    ...
    for node in containers:
        ...
        if any(clean_value(v) for v in rep.values()): reporters.append(rep)
    return reporters
...
def extract_events(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None, debug: bool = False) -> List[Dict[str, Any]]:
    seriousness_map = {"resultsInDeath": "Death","isLifeThreatening": "LT","requiresInpatientHospitalization": "Hospital",
                       "resultsInPersistentOrSignificantDisability": "Disability","congenitalAnomalyBirthDefect": "Congenital",
                       "otherMedicallyImportantCondition": "IME"}
    outcome_map = {"1": "Recovered/Resolved","2": "Recovering/Resolving","3": "Not recovered/Ongoing",
    ...
    try:
        ...
        for rxn in rxns:
            ...
            for crit, label in seriousness_map.items():
                ...
                if crit_el is not None and (crit_el.attrib.get('value') or '').strip().lower() == 'true': flags.append(label)
            ...
            low = rxn.find('.//hl7:effectiveTime/hl7:low', NS); high= rxn.find('.//hl7:effectiveTime/hl7:high', NS)
            ...
            out.append({
                "LLT Code": clean_value(llt_code), "LLT Term": clean_value(llt_term),
                "PT Code": clean_value(pt_code),   "PT Term": clean_value(pt_term),
                "Seriousness": seriousness_disp,   "Outcome": clean_value(outcome),
        ...
        if debug: st.warning(f"Events parse error: {e}")
        return out
...
def extract_model(xml_bytes: bytes, meddra_map: Optional[Dict[str, Dict[str,str]]] = None,
    ...
    except Exception as e: return {"_error": f"XML parse error: {e}"}
    model: Dict[str, Any] = {}
    ...
    model["Causality"] = extract_causality(root, product_id_to_name=product_id_to_name,
                                           reaction_id_to_term=reaction_id_to_term, debug=debug_caus)
...
def make_admin_table(src: Dict[str,Any], prc: Dict[str,Any]) -> pd.DataFrame:
    ...
    parts = [compare_table([rows[0]]), compare_table([rows[1]]), compare_table([rows[2]]), compare_table([rows[3]], treat_as_dates=True)]
...
def make_reporter_pair_table(src_rep: Dict[str,str], prc_rep: Dict[str,str]) -> pd.DataFrame:
    fields = ["Reporter Qualification","Reporter IDs","Reporter Title","Reporter Given Name(s)","Reporter Family Name",
...
def index_by_match_key(products: List[Dict[str,Any]]) -> Dict[str, Dict[str,Any]]:
    ...
    for rec in products:
        ...
        if key and key not in idx: idx[key] = rec
    return idx

def make_drug_compare_table(src_rec: Dict[str,Any], prc_rec: Dict[str,Any]) -> pd.DataFrame:
    fields = ["Type","Dosage Text","Dose Value","Dose Unit","Start Date","Stop Date","Route",
              "Formulation","Lot No","MAH","Action Taken","Drug Obtain Country"]
...
if n_boxes == 0:
    ...
    for i in range(n_boxes):
        ...
        if not r_df.empty: st.table(r_df)
        else: st.markdown('<div class="smallnote">No values for this reporter.</div>', unsafe_allow_html=True)
...
# ---------------- SECTION: Patient — Medical History ----------------
...
src_mh_idx = _idx_mh(src_mh); prc_mh_idx = _idx_mh(prc_mh)
...
def make_mh_box_for_ui(src_rec: Dict[str,Any], prc_rec: Dict[str,Any], title: str):
    ...
    fields = [("LLT Code","text"),("LLT Term","text"),("PT Code","text"),("PT Term","text"),
    ...
    if not mh_df.empty: st.table(mh_df)
    else: st.markdown('<div class="smallnote">No values for this item.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
if not all_mh_keys:
...
def _idx_lab(lst: List[Dict[str,Any]]) -> Dict[str,Dict[str,Any]]:
    return {e.get("_key",""): e for e in lst if e.get("_key","")}
src_lab_idx = _idx_lab(src_lab); prc_lab_idx = _idx_lab(prc_lab)
...
def make_lab_box_for_ui(src_rec: Dict[str,Any], prc_rec: Dict[str,Any], title: str):
    # ✅ Correct, balanced header line (fixes your syntax error)
    ...
    else: st.markdown('<div class="smallnote">No values for this lab item.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
if not all_lab_keys:
...
src_caus = src.get("Causality", []) or []; prc_caus = prc.get("Causality", []) or []
def _key_caus(item: Dict[str,Any]) -> str:
    k = (item.get("_evt_id","") or "") + "::" + (item.get("_prd_id","") or "")
    if not k.strip(":"): k = "assess::" + normalize_text(item.get("Assessment","")) + "::" + normalize_text(item.get("Method",""))
    return k
...
def make_caus_box_for_ui(s: Dict[str,Any], p: Dict[str,Any], title_hint: str):
    ...
    fields = [("Assessment","text"),("Method","text"),("Assessor","text"),("Reaction","text"),("Drug","text")]
    rows = [(f, s.get(f,""), p.get(f,"")) for f, _ in fields]
    df = compare_table(rows, treat_as_dates=False)
    if not df.empty: st.table(df)
    else: st.markdown('<div class="smallnote">No values to display for this causality item.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
if not all_caus_keys:
    st.markdown('<div class="box smallnote">No causality entries found.</div>', unsafe_allow_html=True)
else:
    for key in all_caus_keys:
        se = src_caus_idx.get(key, {}); pe = prc_caus_idx.get(key, {})
        t = (se.get("Reaction") or pe.get("Reaction") or ""); d = (se.get("Drug") or pe.get("Drug") or "")
...
src_prods = src.get("Products", []); prc_prods = prc.get("Products", [])
src_idx = index_by_match_key(src_prods); prc_idx = index_by_match_key(prc_prods)
ordered_keys: List[str] = []; seen_keys = set()
for rec in src_prods:
    k = _drug_match_key(rec)
    if k and k not in seen_keys: seen_keys.add(k); ordered_keys.append(k)
...
if not ordered_keys:
    ...
    for key in ordered_keys:
        ...
        title = (srec.get("Drug") or "").strip() or (prec.get("Drug") or "").strip() or (srec.get("_pid") or "").strip() or (prec.get("_pid") or "").strip() or "(Unnamed drug)"
        ...
        else: st.markdown('<div class="smallnote">No values to display for this drug.</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
src_evts = src.get("Events", []); prc_evts = prc.get("Events", [])
def _idx_events(lst: List[Dict[str,Any]]) -> Dict[str,Dict[str,Any]]:
    return {e.get("_key",""): e for e in lst if e.get("_key","")}
src_evt_idx = _idx_events(src_evts); prc_evt_idx = _idx_events(prc_evts)
...
def make_event_box_for_ui(src_rec: Dict[str,Any], prc_rec: Dict[str,Any], title: str):
    ...
    fields = [("LLT Code","text"),("LLT Term","text"),("PT Code","text"),("PT Term","text"),
              ("Seriousness","text"),("Outcome","text"),("Event Start","date"),("Event End","date")]
    rows = []
    for field, kind in fields:
        s_val = src_rec.get(field,""); p_val = prc_rec.get(field,"")
        if kind == "date":
    ...
    if not e_df.empty: st.table(e_df)
    else: st.markdown('<div class="smallnote">No values to display for this event.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
if not all_evt_keys:
