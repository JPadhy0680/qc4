...
def _extract_assessor_label(node: ET.Element) -> str:
    ...
    for t in cand_texts:
        ...
        if 'reporter' in low: return "Reporter"
    nm = find_first(node, './/hl7:author//hl7:assignedEntity//hl7:name')
    if nm is not None and get_text(nm): return get_text(nm)
    ot = find_first(node, './/hl7:author//hl7:assignedEntity//hl7:originalText')
    if ot is not None and get_text(ot): return get_text(ot)
...
def extract_all_products(root: ET.Element, meddra_map: Optional[Dict[str, Dict[str,str]]] = None) -> List[Dict[str, Any]]:
    """
    Windowed per‑SA extraction.
    One product row per <substanceAdministration classCode="SBADM" moodCode="EVN"> in document order.
    SA-native fields are read from the SA subtree; window-scoped fields are searched within the
    same <component typeCode="COMP"> between this SA and the next SA SBADM/EVN sibling.
    """
    ...
    for cidx, comp in enumerate(comps, start=1):
        # Prefer SAs that are direct children of this component
        comp_children = list(comp)
        sa_nodes_direct = [n for n in comp_children
                           if local_name(n.tag) == 'substanceAdministration'
                           and n.attrib.get('moodCode') == 'EVN'
                           and n.attrib.get('classCode') == 'SBADM']
        sa_nodes = sa_nodes_direct
        if not sa_nodes:
            # Fallback to descendant search when structure is not flat
            sa_nodes = comp.findall('.//hl7:substanceAdministration[@moodCode="EVN"][@classCode="SBADM"]', NS)

        for sa_idx, sa in enumerate(sa_nodes, start=1):
            # Build window within this component: SA .. next SA (exclusive)
            if sa in comp_children:
                start_pos = comp_children.index(sa)
                end_pos = len(comp_children)
                for j in range(start_pos + 1, len(comp_children)):
                    cand = comp_children[j]
                    if local_name(cand.tag) == 'substanceAdministration' \
                       and cand.attrib.get('moodCode') == 'EVN' \
                       and cand.attrib.get('classCode') == 'SBADM':
                        end_pos = j
                        break
                window_nodes = comp_children[start_pos:end_pos]
            else:
                # If SA not a direct child (descendant fallback), limit window to SA only
                window_nodes = [sa]

            def win_findall(xpath: str):
                acc = []
            ...
            title = clean_value(_resolve_drug_name(sa)) or (pid or f"sa::{cidx:03d}-{sa_idx:03d}")
            ...
            # SA-native fields
            dosage_text = get_text(find_first(sa, './/hl7:text'))
            dq = find_first(sa, './/hl7:doseQuantity')
            dose_val  = (dq.attrib.get('value') or '').strip() if dq is not None else ''
            dose_unit = (dq.attrib.get('unit')  or '').strip() if dq is not None else ''
            low  = find_first(sa, './/hl7:effectiveTime/hl7:low')
            high = find_first(sa, './/hl7:effectiveTime/hl7:high')
            start_date = format_date((low.attrib.get('value') or '').strip()  if low  is not None else '')
            stop_date  = format_date((high.attrib.get('value') or '').strip() if high is not None else '')

            route_txt = get_text(find_first(sa, './/hl7:routeCode/hl7:originalText'))
            if not route_txt:
                rc = find_first(sa, './/hl7:routeCode')
                route_txt = (rc.attrib.get('displayName') or '').strip() if rc is not None else ''

            form_txt = get_text(find_first(sa, './/hl7:formCode/hl7:originalText'))
            lot_no   = get_text(find_first(sa, './/hl7:lotNumberText'))
            ...
            for act_code in win_findall('.//hl7:act[@classCode="ACT"][@moodCode="EVN"]/hl7:code'):
                if (act_code.attrib.get('codeSystem') or '').strip() == ACTION_TAKEN_OID:
                    ...
                    if label and label not in action_taken_vals:
                        action_taken_vals.append(label)
            action_taken = '\n'.join(action_taken_vals)

            obtain_countries = []
            for cn in win_findall('.//hl7:country'):
                val = (cn.text or '').strip()
                if val and val not in obtain_countries:
                    obtain_countries.append(val)
            obtain_country = '\n'.join(obtain_countries)
            ...
            for ind_obs in win_findall('.//hl7:observation'):
                ...
                if cs == STATUS_OID and (dn == 'indication' or cd == '19'):
                    ...
                    if val is not None:
                        ...
                        if frag and frag not in indications:
                            indications.append(frag)
            indication_txt = '\n'.join(indications)

            out.append({
                'Drug': title,
                'Type': type_disp,
                'Dosage Text': clean_value(dosage_text),
                'Dose Value': clean_value(dose_val),
                'Dose Unit': clean_value(dose_unit),
                'Start Date': clean_value(start_date),
                'Stop Date': clean_value(stop_date),
                'Route': clean_value(route_txt),
                'Formulation': clean_value(form_txt),
                'Lot No': clean_value(lot_no),
                'MAH': clean_value(mah),
                'Action Taken': clean_value(action_taken),
                'Drug Obtain Country': clean_value(obtain_country),
                'Indication': clean_value(indication_txt),
                '_gid': f"pid::{pid.lower()}" if pid else f"sa::{cidx:03d}-{sa_idx:03d}",
                '_pid': pid or '',
            })
